      ...
      <section className="relative mx-auto flex min-h-[calc(100vh-3rem)] max-w-7xl flex-col">
        <header className="flex items-center justify-between border-b border-white/8 pb-4">
          <div className="flex items-center gap-3">
            <div className="grid size-10 place-items-center rounded-xl border border-cyan-300/20 bg-cyan-300/5 text-cyan-200"><Sparkles size={18} /></div>
            <div><div className="text-sm font-semibold tracking-[0.28em] text-white">J.A.R.V.I.S.</div><div className="text-[10px] tracking-[0.2em] text-slate-500">CORE INTELLIGENCE / 2026</div></div>
          </div>
          <div className="flex items-center gap-4 text-[11px] tracking-[0.15em] text-slate-400"><span className="flex items-center gap-2"><Activity size={13} /> {status}</span><span className="hidden items-center gap-2 sm:flex"><ShieldCheck size={13} /> PRIVATE CORE</span></div>
        </header>
        ...
        <div className="grid flex-1 gap-6 py-8 lg:grid-cols-[260px_minmax(0,1fr)_260px]">
          <aside className="panel hidden rounded-2xl p-5 lg:block">
            ...
            <div className="space-y-3 text-sm">{["Reasoning Core", "Context Engine", "Tool Router", "Memory Layer"].map((item) => <div key={item} className="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.02] px-3 py-3 text-slate-300"><span>{item}</span><span className="size-1.5 rounded-full bg-emerald-300 shadow-[0_0_12px_#5af1b8]" /></div>)}</div>
          </aside>
