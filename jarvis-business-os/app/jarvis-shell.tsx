...
import { Activity, ArrowUpRight, BrainCircuit, CalendarClock, CheckCircle2, Cpu, Database, Github, Mic, Send, ShieldCheck, Sparkles, X } from "lucide-react";
    ...
    <section className="relative mx-auto max-w-7xl">
      ...
      <div className="grid gap-5 xl:grid-cols-[250px_minmax(0,1fr)_290px]">
        ...
        <section className="panel flex min-h-[630px] flex-col rounded-2xl">
          <div className="flex items-start justify-between border-b border-white/7 px-5 py-4"><div><div className="text-xs tracking-[.22em] text-cyan-200/75">EXECUTIVE COPILOT</div><h1 className="mt-1 text-2xl font-medium text-white">¿Qué resolvemos hoy?</h1></div><div className="hidden rounded-full border border-emerald-300/15 bg-emerald-300/5 px-3 py-1 text-[10px] tracking-[.14em] text-emerald-200 sm:block">SESSION PROTECTED</div></div>
          <div className="flex-1 space-y-4 overflow-y-auto px-5 py-6">{messages.length === 0 ? <div className="flex h-full flex-col items-center justify-center text-center"><div className="orb mb-8" aria-label="JARVIS core visualizer" /><p className="max-w-md text-sm leading-6 text-slate-400">Planifica tu día, sintetiza información, conserva contexto y usa herramientas reales cuando estén conectadas.</p><div className="mt-5 flex flex-wrap justify-center gap-2">{["Resume mis prioridades", "Recuerda mi objetivo", "Estado de integraciones"].map((prompt) => <button key={prompt} onClick={() => setInput(prompt)} className="rounded-full border border-cyan-300/15 px-3 py-2 text-xs text-cyan-100/80 transition hover:border-cyan-300/40 hover:bg-cyan-300/5">{prompt}</button>)}</div></div> : messages.map((message, index) => <div key={`${message.role}-${index}`} className={`max-w-[92%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "ml-auto bg-cyan-300/12 text-cyan-50" : "border border-white/6 bg-white/[.035] text-slate-200"}`}>{message.content || "…"}</div>)}</div>
